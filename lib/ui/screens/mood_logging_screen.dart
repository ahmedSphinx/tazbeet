...
class _MoodLoggingScreenState extends State<MoodLoggingScreen> {
  ...
  void _submitMood() {
    if (_selectedMoodLevel == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please select a mood level')));
      return;
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Padding(
        ...
        child: ListView(
          children: [
            Text(AppLocalizations.of(context)!.howAreYouFeeling, style: TextStyle(fontSize: 18)),
            ...
            const SizedBox(height: 24),
            _buildSlider(AppLocalizations.of(context)!.energyLevel, /* 'Energy Level', */ _energyLevel, (value) {
              setState(() {
                _energyLevel = value.toInt();
              });
            }),
            const SizedBox(height: 16),
            _buildSlider(AppLocalizations.of(context)!.focusLevel, /* 'Focus Level', */ _focusLevel, (value) {
              setState(() {
                _focusLevel = value.toInt();
              });
            }),
            const SizedBox(height: 16),
            _buildSlider(AppLocalizations.of(context)!.stressLevel, /* 'Stress Level', */ _stressLevel, (value) {
              setState(() {
                _stressLevel = value.toInt();
              });
            }),
            const SizedBox(height: 32),
            ElevatedButton(onPressed: _submitMood, child: Text(AppLocalizations.of(context)!.saveMood)),
          ],
        ),
      ),
    );
  ...
  Widget _buildSlider(String label, int value, ValueChanged<double> onChanged) {
    return Column(
      ...
      children: [
        Text('$label: $value/10'),
        Slider(value: value.toDouble(), min: 1, max: 10, divisions: 9, label: value.toString(), onChanged: onChanged),
      ],
    );
  }
