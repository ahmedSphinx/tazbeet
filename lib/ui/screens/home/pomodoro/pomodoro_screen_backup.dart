...
class PomodoroScreenRedesigned extends StatefulWidget {
  final Task? initialTask;
  final PomodoroTemplate? template;
  final VoidCallback? onTimerStopped;

  const PomodoroScreenRedesigned({super.key, this.initialTask, this.template, this.onTimerStopped});

  @override
  State<PomodoroScreenRedesigned> createState() => _PomodoroScreenRedesignedState();
}

class _PomodoroScreenRedesignedState extends State<PomodoroScreenRedesigned> with TickerProviderStateMixin {
  ...
  void _onTimerChanged() {
    if (mounted) {
      setState(() {
        // Update completed sessions if timer was idle and now has sessions
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              Text('You\'ve completed all ${_currentTemplate.cycles} cycles of the ${_currentTemplate.name} template!', style: TextStyle(fontSize: 16, color: isDark ? colorScheme.onSurface : Colors.black87)),
              const SizedBox(height: 8),
              Text('Total work time: ${_workTimeMinutes} minutes', style: TextStyle(fontSize: 14, color: isDark ? colorScheme.onSurfaceVariant : Colors.grey[600])),
            ],
          ),
          actions: [
            TextButton(
            ...
            ElevatedButton(
              ...
              child: const Text('Restart Cycles'),
            ),
          ],
        );
      },
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    ...
                    Expanded(child: _buildBottomControlsArea()),
                  ],
                ),
              ),
            ),
    ...
    return Row(
      children: [
        ...
        Column(
          ...
          children: [
            Text(
              '${_currentTemplate.workDuration}min',
              style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: isDark ? colorScheme.onSurface : Colors.black87),
            ),
            Text('${_currentTemplate.cycles} cycles', style: TextStyle(fontSize: 11, color: isDark ? colorScheme.onSurfaceVariant : Colors.grey[600])),
          ],
        ),
      ],
    );
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        const SizedBox(width: 4),
                        Text('Session ${_timer.currentSession}/${_currentTemplate.cycles}', style: TextStyle(fontSize: 12, color: isDark ? colorScheme.onSurfaceVariant : Colors.grey[600])),
                      ],
                    ),
                  ],
                ),
  ...
  Widget _buildQuickActionsRow() {
    return Row(
      ...
      children: [
        ...
        _buildQuickActionButton(icon: Icons.skip_next, label: 'Skip', onTap: () => _skipCurrentSession(), color: Colors.orange),
      ],
    );
  }
        ...
        children: [
          Row(
            ...
            children: [
              ...
              Text('${_completedSessions}/${_currentTemplate.cycles} completed', style: TextStyle(fontSize: 12, color: isDark ? colorScheme.onSurfaceVariant : Colors.grey[600])),
            ],
          ),
          const SizedBox(height: 12),
          ...
          const SizedBox(height: 12),
          Row(mainAxisAlignment: MainAxisAlignment.spaceAround, children: [_buildStatItem('Work Time', '${_workTimeMinutes}min', Icons.work), _buildStatItem('Break Time', '${_breakTimeMinutes}min', Icons.coffee), _buildStatItem('Total', '${_workTimeMinutes + _breakTimeMinutes}min', Icons.schedule)]),
        ],
      ),
    );
  }
    ...
    return Column(
      children: [
        ...
        Row(
          children: [
            Expanded(
              child: _buildSecondaryButton(icon: Icons.refresh, label: 'Reset', onTap: _resetTimer),
            ),
            const SizedBox(width: 12),
            Expanded(
              child: _buildSecondaryButton(icon: Icons.stop, label: 'Stop', onTap: _stopTimer),
            ),
          ],
        ),
      ],
  ...
  String _getAppBarTitle() {
    if (widget.initialTask != null) {
      return widget.initialTask!.title;
    }
  ...
  void _toggleTimer() {
    switch (_timer.state) {
      ...
      case PomodoroState.longBreak:
        ...
        break;
      case PomodoroState.paused:
        _timer.start(); // Use start instead of resume since resume doesn't exist
        break;
      default:
        _timer.start();
    }
  ...
  void _adjustTime(int minutes) {
    if (_timer.state != PomodoroState.idle) {
      _timer.addTime(minutes); // addTime handles both positive and negative values
    }
  }

  void _skipCurrentSession() {
    _timer.skip(); // Use the available skip method
  }

  void _resetTimer() {
    _timer.stop(); // Use stop method since reset doesn't exist
    setState(() {
      _completedSessions = 0;
      _workTimeMinutes = 0;
      _breakTimeMinutes = 0;
  ...
  String _getTemplateTypeLabel() {
    if (_currentTemplate.recommendedFor == 'adhd') return 'ADHD Recommended';
    if (_currentTemplate.recommendedFor == 'normal') return 'Standard';
    if (_currentTemplate.isCustom) return 'Custom Template';
    return 'General';
  }
}
