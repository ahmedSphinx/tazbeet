...
class _ProgressScreenState extends State<ProgressScreen> {
  ...
  Widget _buildProgressContent(List<Task> tasks, CategoryState categoryState) {
    ...
    // Filter out tasks with null values and ensure data integrity
    final validTasks = tasks.where((task) => task.isCompleted == true).toList();
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          _buildOverviewCard(validTasks),
          const SizedBox(height: 24),
          _buildWeeklyProgressCard(validTasks),
          const SizedBox(height: 24),
  ...
  Widget _buildOverviewCard(List<Task> tasks) {
    final todayProgress = _progressService.getDailyCompletionPercentage(tasks, DateTime.now());
    final weekProgress = _progressService.getWeeklyCompletionPercentage(tasks, _getWeekStart());
    final monthProgress = _progressService.getMonthlyCompletionPercentage(tasks, _getMonthStart());
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(AppLocalizations.of(context)!.overview, style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.bold)),
            const SizedBox(height: 16),
            Row(
              children: [
                Expanded(child: _buildProgressMetric(AppLocalizations.of(context)!.today, '${(todayProgress * 100).round()}%', todayProgress, Icons.today)),
                const SizedBox(width: 16),
                Expanded(child: _buildProgressMetric(AppLocalizations.of(context)!.week, '${(weekProgress * 100).round()}%', weekProgress, Icons.calendar_view_week)),
              ],
            ),
            const SizedBox(height: 16),
            Row(
              children: [
                Expanded(child: _buildProgressMetric(AppLocalizations.of(context)!.month, '${(monthProgress * 100).round()}%', monthProgress, Icons.calendar_month)),
                const SizedBox(width: 16),
                Expanded(
  ...
  Widget _buildProgressMetric(String label, String value, double progress, IconData icon, {Color? color}) {
    return Column(
      children: [
      ...
      children: [
        Row(
          children: [
            ...
            Text(AppLocalizations.of(context)!.productivityScore, style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w600)),
          ],
        ),
        const SizedBox(height: 8),
        Row(
          children: [
            Expanded(
              child: LinearProgressIndicator(value: score, backgroundColor: Theme.of(context).colorScheme.surfaceContainerHighest, valueColor: AlwaysStoppedAnimation<Color>(Theme.of(context).colorScheme.secondary)),
            ),
            const SizedBox(width: 12),
            Text(
              '$percentage%',
              style: Theme.of(context).textTheme.bodyLarge?.copyWith(fontWeight: FontWeight.bold, color: Theme.of(context).colorScheme.secondary),
            ),
          ],
        ),
      ],
  ...
  Widget _buildDayBar(String day, double progress) {
    return Column(
      ...
      children: [
        Container(
          width: 32,
          height: (progress * 80).clamp(4, 80), // Min height of 4 for visibility
  ...
  Widget _buildCategoryProgressCard(List<Task> tasks, List<Category> categories) {
    if (categories.isEmpty) return const SizedBox.shrink();

    // Filter out tasks with null values and ensure data integrity
    final validTasks = tasks.where((task) => task.isCompleted == true).toList();

    final categoryProgress = _progressService.getCategoryProgress(validTasks, categories.map((c) => c.id).toList());
              ...
              return Padding(
                ...
                child: Row(
                  children: [
                    ...
                    const SizedBox(width: 12),
                    Expanded(child: Text(category.name, style: Theme.of(context).textTheme.bodyMedium)),
                    Text('${(progress * 100).round()}%', style: Theme.of(context).textTheme.bodyMedium?.copyWith(fontWeight: FontWeight.w600)),
                    const SizedBox(width: 12),
                    Expanded(
                      flex: 2,
                      child: LinearProgressIndicator(value: progress, backgroundColor: Theme.of(context).colorScheme.surfaceContainerHighest),
                    ),
                  ],
                ),
              );
  ...
  Widget _buildStatsCard(List<Task> tasks) {
    ...
    final completedTasks = validTasks.length;
          ...
          children: [
            ...
            Row(
              children: [
                ...
                const SizedBox(width: 16),
                Expanded(child: _buildStatItem(AppLocalizations.of(context)!.completedLabel, completedTasks.toString(), Icons.check_circle, color: Colors.green)),
              ],
            ),
            const SizedBox(height: 16),
            Row(
